# API de Catálogo de Livros (Tech Challenge)
...
### Descrição
Esta é uma API RESTful desenvolvida em Python com o framework Flask. A aplicação permite o gerenciamento de um catálogo de livros, incluindo um sistema de autenticação de usuários via JWT (JSON Web Tokens). A principal funcionalidade é a capacidade de popular o banco de dados de livros através de um processo de web scraping, que pode ser acionado por um endpoint específico.
...
│   ├── models.py        # Definição das tabelas (User, Book)
│   ├── routes.py        # Rotas principais (User, Auth, Search)
